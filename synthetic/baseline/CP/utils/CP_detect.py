...
def compute_cpp(collusion_P, percentage,  epoch):
    ...
    for i in range(len(worker)):
        print("current worker:", worker.iloc[i][0])
...
#
# # 评价标准2
# def TPFN_v2(exp, epoch, _list):
#     # 串谋工人
#     worker_partner = _list
#
#     worker_ture = []
#     worker_ture_pd = pd.read_csv("../../../experience/result/experience_" + str(exp) + "/data" + str(epoch) + "_collaborate_W.csv")
#     for i in range(len(worker_ture_pd)):
#         worker_ture.append(worker_ture_pd.iloc[i][0])
#
#     # print("预测串谋工人：", sorted(worker_pre))
#     # print("真实串谋工人：", sorted(worker_ture))
#
#     # 找出串谋工人所有的串谋任务
#     # 找出每个工人所有的任务，
#     data = pd.read_csv("../../../experience/result/experience_" + str(exp) + "/data" + str(epoch) + "_collaborate_D.csv")
#
#     # 找出真实串谋任务
#     works_true = process(data, worker_ture)
#
#     # 找出预测串谋任务
#     works_pre = process(data, worker_partner)
#
#     TP = 0
#
#     # 求 TP  FN
#     for i in range(len(works_true)):
#         # print(i)
#         for j in range(len(works_pre)):
#             df1 = works_true.iloc[i:i+1, :]
#             df2 = works_pre.iloc[j:j+1, :]
#             if df1.equals(df2):
#                 TP = TP + 1
#
#     FN = len(works_true) - TP
#     FP = len(works_pre) - TP
#     TN = len(data) - TP - FP - FN
#
#     print("TP=", TP, "/", len(works_true))
#     print("FN=", FN, "/", len(works_true))
#     print("FP=", FP, "/", len(data) - len(works_true))
#     print("TN=", TN, "/", len(data) - len(works_true))
#
#     Accuracy = (TP + TN) / len(data)
#     if TP + FP != 0:
#         Precsion = TP / (TP + FP)
...
#
#
# def ACC_PRE_REC(exp, epoch, 超参数):
#
#     print("exp=", exp, " - epoch=", epoch, " - 超参数=", 超参数)
#
#     path = "../result/WPCR/wpcr_"+str(exp)+"_"+str(epoch)+".csv"
#
#     collosion_pd = pd.read_csv(path)
#
#     collosion_worker = []
#
#     # print(collosion_worker)
#     for i in range(len(collosion_pd)):
#         if collosion_pd.iloc[i][1] >= 超参数:
#             collosion_worker.append(int(collosion_pd.iloc[i][0]))
#
#     print(collosion_worker)
#
#     print("评价窗口1 ： acc recall....")
#     (acc1, pre1, rec1) = TPFN_v1(exp, epoch, collosion_worker)
#
#     print("------------------------------------------")
#
#     print("评价窗口2 ： acc recall....")
#     (acc2, pre2, rec2) = TPFN_v2(exp, epoch, collosion_worker)
#     print("===========================================")
#
#     return (acc1, pre1, rec1, acc2, pre2, rec2)
#
#
# def display_ACC_PRE_REC(x, 超参数, epoch):
#
#     Acc1 = []
#     Pre1 = []
#     Rec1 = []
...
#
#         for epo in range(epoch, epoch+5):
#             (acc1, pre1, rec1, acc2, pre2, rec2) = ACC_PRE_REC(x[i], epo, 超参数)
#
#             Acc1.append(acc1)
#             Pre1.append(pre1)
#             Rec1.append(rec1)
...
#
#     Acc2 = DataFrame({"Acc": Acc2})
#     Pre2 = DataFrame({"Pre": Pre2})
#     Rec2 = DataFrame({"Rec": Rec2})
#
#
#     write1 = concat([Acc1, Pre1, Rec1], axis=1)
#     write1.to_csv(
#         "../result/threshold_" + str(超参数) + "/experience1_result_" + str(
#             x[0]) + "_" + str(epoch) + ".csv", sep=',', index=0)
#
#     write2 = concat([Acc2, Pre2, Rec2], axis=1)
#     write2.to_csv(
#         "../result/threshold_" + str(超参数) + "/experience2_result_" + str(
#             x[0]) + "_" + str(epoch) + ".csv", sep=',', index=0)
#
...
def ZB(collusion_P, per, epo):
    ...
    print("FC: collusion_P=", collusion_P, "percentage=", per, "epoch", epo)
...
if __name__ == '__main__':
    ...
    # 生成每一个工人的WPCR
    # display_CPP(collusion_P, KCDN_percentage, 0)
