...
def E2(percentage, epo, collusion_P):
    ...
    print("collusion_P=",collusion_P, "KCDN_percentage=",percentage, "epo=", epo)
    # print("|")
    # 获取数据集
    path = "../../result/collusion_data/collusion_"+str(collusion_P)+"/experience_"+str(percentage)+"/data"+str(epo)+"_collaborate_D.csv"
    save_path = "../../result/collusion_data/collusion_"+str(collusion_P)+"/experience_"+str(percentage)+"/data"+str(epo)+"_collaborate_wjj.csv"
...
def save_worker_ks_sumW_sumWN(collusion_P, percentage, epo, worker_sumW_sumWN, worker_ture_pd):
    ...
    # print(write)
    savepath = "../../result/collusion_data/collusion_"+str(collusion_P)+"/experience_"+str(percentage)+"/data"+str(epo)+"_collaborate_worker_ks_sumW_sumWN.csv"
...
# # 计算并得到【workers,ks,sum_W,sum_W,find】 ，保存
def get_sumW_sumWN(collusion_P, percentage, epo):

    print(collusion_P, percentage, epo)
    """得到每条边的权重"""
    path = "../../result/collusion_data/collusion_"+str(collusion_P)+"/experience_"+str(percentage)+"/data"+str(epo)+"_collaborate_wjj.csv"
    worker_ture = []
    """当前实验epoch 的 真正串谋者"""
    worker_ture_pd = pd.read_csv("../../result/collusion_data/collusion_"+str(collusion_P)+"/experience_"+str(percentage)+"/data"+str(epo)+"_collaborate_W.csv")
...
def sumW_sumWN(collusion_P, percentage, epoch):
    ...
    for exp in range(len(percentage)):
        for ep0 in range(epoch, epoch+1):
            # 计算并得到【workers,ks,sum_W,sum_W,find】 ，保存
...
if __name__ == '__main__':
    ...
    percentage = [12,24,36,48,60]  # 串谋占比
