...
def Percentage(collusion_P, pre, alpha, beta, K, epoch):
    ...
    print("collusion_P=", collusion_P,"alpha=", alpha, "beta=", beta, "KCDN_percentage=", pre, "dection=", pre, "epoch=", epoch)

    # print(path)
    path = "../../result/collusion_data/collusion_"+str(collusion_P)+"/experience_"+str(pre)+"/data"+str(epoch)+"_collaborate_worker_ks_sumW_sumWN.csv"
...
def display_Percentage(collusion_P, percentage, alpha, beta, K, epoch):
    ...
    for pre in range(len(percentage)):
        ...
        save_path = "../../result/collusion_data/collusion_0.8/KCD_percentage/collusion_detect_" + str(
            percentage[pre]) + ".csv"
