...
def GLAD(list, truth_file):

    list = list
    W = np.zeros((len(list), 20))

    for i in range(len(list)):

        for j in range(20):
            print(list[i], j, "/", 20)
            datafile = "../../experience/result/experience_" + str(list[i]) + "/data" + str(j) + "_collaborate_D.csv"
            # print(path)
            ...
            W[i][j] = accuracy
            #
        np.savetxt("../../experience/acc_result/GLAD_acc_result"+str(i)+".csv", W, delimiter=",", fmt='%.8f')
...
def MV(list, gt_data):
    ...
    for i in range(len(list)):
        for j in range(30):
            print(list[i], j, "/", 30)
            path = "../../experience/result/experience_" + str(list[i]) + "/data" + str(j) + "_collaborate_D_del_t7.csv"
            # print(path)
            ...
            np.savetxt("../../experience/acc_result/MV_acc_result_del_t712.csv", W, delimiter=",", fmt='%.8f')
...
if __name__ == '__main__':
    ...
    datafile = "../../../data/createData.csv"

    model = HDS(datafile, truth_file=truth_file)
    model.run()
    accuracy = model.get_accuracy()
    print("HDS准确率：%f" % (accuracy))


    # gt = pd.read_csv(truth_file)
    # data = pd.read_csv(datafile)
    # accuracy = get_MV_acc(data, gt)
    ...
    # glad.run()
    # accuracy = glad.get_accuracy()
    # print('The accuracy of GLAD is %f' % accuracy)
    # The accuracy of GLAD is 0.835192