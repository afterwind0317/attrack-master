        ...
        for b in range(len(beta)):
            for g in range(len(gama)):
                path = "../../collusion_P_0.8/KCDN_W(废)/a_b_r/a" + str(alpha[a]) + "_b" + str(beta[b]) + "_r" + str(
                    gama[g]) + "_KCDN_Percentage_32_0.csv"
...
def pltData(target):
    ...
    sns.heatmap(target_data, annot=True, fmt="0.3f", vmin=min_number, vmax=max_number, cmap='Reds', yticklabels=y_axis,
                xticklabels=x_axis)

    plt.title(target + ":  alpha = 1 ", size=26)
    plt.xlabel('gama', labelpad=20, size=22)

    plt.ylabel('beta', labelpad=20, size=22)
