...
if __name__ == '__main__':
    ...
    # 固定串谋占比为 30%， 讨论串谋概率的影响[0.6,0.7,0.8,0.9,1.0]
    percentage = 36
    ...
    # 获取串谋数据及的分布情况
    collusion_result_6 = []
    collusion_result_6 = get_collusion_result(0.6, percentage)
    print(0.6)

    # 获取串谋数据及的分布情况
    collusion_result_7 = []
    collusion_result_7 = get_collusion_result(0.7, percentage)
    print(0.7)

    # 获取串谋数据及的分布情况
    collusion_result_8 = []
    collusion_result_8 = get_collusion_result(0.8, percentage)
    print(0.8)

    # 获取串谋数据及的分布情况
    collusion_result_9 = []
    collusion_result_9 = get_collusion_result(0.9, percentage)
    print(0.9)

    # 获取串谋数据及的分布情况
    collusion_result_10 = []
    collusion_result_10 = get_collusion_result(1.0, percentage)
    print(1.0)

    x = np.arange(0, 1, 0.01)
    plt.plot(x, resource_result, color='red', label='origin', )
    plt.plot(x, collusion_result_6, color='blue', label='P=0.6', )
    plt.plot(x, collusion_result_7, color='gray', label='P=0.7', )
    plt.plot(x, collusion_result_8, color='yellow', label='P=0.8', )
    plt.plot(x, collusion_result_9, color='black', label='P=0.9', )
    plt.plot(x, collusion_result_10, color='c', label='P=1.0', )
    ...
    plt.xlabel(u"Ability of worker(synthetic)", size=14)  # X轴标签
    plt.ylabel("Density", size=14)  # Y轴标签

    path = 'Beta_synthetic_P.jpg'
