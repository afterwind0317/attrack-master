...
def access(P):
    ...
    for i in range(len(list)):
        save_path = "../synthetic/collusion_" + str(P) + "/access_" + str(list[i]) + ".csv"
        list_id = []
        list_W = []
        list_S = []
        list_Si = []
        for j in range(30):
            list_id.append(j)
            W_data = pd.read_csv("../synthetic/collusion_" + str(P) + "/experience_" + str(list[i]) + "/data" + str(
                j) + "_collaborate_W.csv")
            list_W.append(len(W_data))
            S_data = pd.read_csv("../synthetic/collusion_" + str(P) + "/experience_" + str(list[i]) + "/data" + str(
                j) + "_collaborate_S.csv")
            list_S.append(len(S_data))
            Si_data = pd.read_csv("../synthetic/collusion_" + str(P) + "/experience_" + str(list[i]) + "/data" + str(
...
def salary():
    ...
    Var_U6 = [0]  # 普通工人收益方差
    for i in range(len(list)):
        path = "../synthetic/collusion_0.6/access_" + str(list[i]) + ".csv"
    ...
    Var_U7 = [0]  # 普通工人收益方差
    for i in range(len(list)):
        path = "../synthetic/collusion_0.7/access_" + str(list[i]) + ".csv"
        data = pd.read_csv(path)
    ...
    Var_U8 = [0]  # 普通工人收益方差
    for i in range(len(list)):
        path = "../synthetic/collusion_0.8/access_" + str(list[i]) + ".csv"
        data = pd.read_csv(path)
    ...
    Var_U9 = [0]  # 普通工人收益方差
    for i in range(len(list)):
        path = "../synthetic/collusion_0.9/access_" + str(list[i]) + ".csv"
        data = pd.read_csv(path)
    ...
    Var_U10 = [0]  # 普通工人收益方差
    for i in range(len(list)):
        path = "../synthetic/collusion_1.0/access_" + str(list[i]) + ".csv"
        data = pd.read_csv(path)
    ...
    plt.xlabel(u"Collusion Percentage(synthetic)", size=14)  # X轴标签
    plt.ylabel("Utility", size=14)  # Y轴标签

    plt.tick_params(direction='in')

    path = 'Utility_synthetic.jpg'
